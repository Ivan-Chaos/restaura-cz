...
- **CTA destinations are configuration**: `NEXT_PUBLIC_SIGNUP_URL` and `NEXT_PUBLIC_NOTIFY_URL` (both support `{locale}` and `{plan}` placeholders). A call to action must never resolve to `#` — an e2e test enforces it. Unset, **"start for free" goes to the app's own `/sign-up`**; "notify me" still falls back to a `mailto:`, because the paid tiers genuinely have nothing to sign up to. `SIGNUP_PATH` is deliberately locale-less: `CtaButton` hands an internal href to the localised `Link`, which adds the prefix, so `/cs/sign-up` here would come out as `/cs/cs/sign-up`. `NEXT_PUBLIC_SITE_URL` sets `metadataBase`, without which a relative Open Graph image is a build error.
